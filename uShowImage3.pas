...
uses
  ...
  uDepGrafList1, ComCtrls, uAbstrGeometry, uAbstrArray, Math, uMsgDial, uEditParam2v2,
  Grids, uEditParamConst, uDEPdescript2, uDEPgrafJob2, uDBVgrafQuery, Printers;
...
//    gWdCopy: TprqRptGrafWindow;
  ...
  public
    ...
    fgWind: TprqRptGrafWindow; // Окно, соответствующее треку
...
// Указать интервал
    ...
    function _findWindow(pnt1: TPoint): TprqRptGrafWindow;
  public
    { Public declarations }
...
procedure TcShowImage3.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TcShowImage3.Init(jobParam: TDEPgrafJob1; depImgParam: TdepImgParam;
...
procedure TcShowImage3.FormShow(Sender: TObject);
begin
  inherited;
  if self.bFirst then
  begin
...
procedure TcShowImage3.Panel1Resize(Sender: TObject);
begin
  inherited;
...
procedure TcShowImage3.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
...
procedure TcShowImage3.picDown;
var
  dtEnd: double;
begin
  inherited;
...
procedure TcShowImage3.BitBtn2Click(Sender: TObject);
begin
  inherited;
...
procedure TcShowImage3.BitBtn3Click(Sender: TObject);
begin
  inherited;
  self.bigStep := self.CheckBox1.Checked;
  picDown;
...
procedure TcShowImage3.Image1MouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  inherited;
...
procedure TcShowImage3.setDelta;
begin
...
procedure TcShowImage3.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  Step: Boolean;
begin
...
procedure TcShowImage3.StateMouseDefine(Shift: TShiftState; X, Y: Integer);
var
  pnt1: TPoint;
begin
  inherited;
  ...
  self.DescriptMouse.fgWind := self._findWindow(pnt1); // self.fImage1[self.fImage1.findWindowList(self.numList)].gWd;
  if self.DescriptMouse.fgWind = nil then Exit;
  ...
  self.DescriptMouse.dTime    := self.DescriptMouse.fgWind.dTimeScale.getTimefromY(self.DescriptMouse.pntLog.Y);
...
procedure TcShowImage3.Image1MouseDown(Sender: TObject;
...
procedure TcShowImage3.Image1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  inherited;
...
procedure TcShowImage3.StateMouseShow;
begin
{
...
procedure TcShowImage3.ComboBox1Change(Sender: TObject);
begin
  inherited;
  self.fEtapEditMode := 1; //self.ComboBox1.ItemIndex;
end;

class function TcShowImage3.isValideList(gList: TprqRptGrafList1): Boolean;
var
  jC1: Integer;
begin
  result := true;
...
procedure TcShowImage3.BitBtn4Click(Sender: TObject);
var
//  Msg: TMessage;
...
//        jNom := 1;
        ...
        self.fImage1[1].img.Picture.Bitmap.SaveToFile(s1);
    end;
...
procedure TcShowImage3.RepaintImage(Sender: TObject);
begin
  fSource := Image1.ClientRect;
  yShift := 0;
...
procedure TcShowImage3.LineRepaint(const pnt: TPoint; Width: Integer;
  Color: TColor);
var
  pnt1: TPoint;
...
//  fgWind := self.fImage1[self.fImage1.findWindowList(self.numList)].gWd;
  ...
  pnt1.X := fJob.gList.logToPointX(fJob.gList.LogOtsLeft + pc005_505_App_Shift1 + pc005_505_App_Shift2 + pc005_505_App_Shift5);
  pnt1.Y := pnt.Y;

  Image1.Picture.Bitmap.Canvas.MoveTo(pnt1.X,  pnt1.Y);

  pnt1.X := fJob.gList.logToPointX(fJob.gList.LogWidth - fJob.gList.LogOtsRight - pc005_505_App_Shift1);
...
procedure TcShowImage3.intervalLineRepaint(const pnt: TPoint);
begin
  LineRepaint(pnt, 3, pc005_110_CB01_05cl);
end;

procedure TcShowImage3._NewInterval_D;
var
j1: Integer;
begin // MouseDown...
...
procedure TcShowImage3._NewInterval_M;
begin
  if not ([ssLeft] = self.DescriptMouse.pntShift) then Exit;
...
procedure TcShowImage3._NewInterval_U;
begin
...
procedure TcShowImage3.IntervalRepaint;
var
  pnt1, pnt2: TPoint;
  jC1: Integer;
  fgWind: TprqRptGrafWindow;
...
procedure TcShowImage3.intervalLineRepaint(const dTime: Double);
var
  pnt1, pnt2: TPoint;
  fgWind: TprqRptGrafWindow;
begin
...
procedure TcShowImage3.BitBtn5Click(Sender: TObject);
var
...
begin
  ...
  jN := self.fImage1.Count;
  if jN > 1 then
  begin
    InputString := Trim(InputBox(self.Caption, 'Укажите номер трека от 1 до ' + IntToStr(jN), ''));
    if Trim(InputString) = '' then
    begin
      Exit;
    end;
    if uSupport.prqStrToInt(InputString, j1) then
    begin
      if (j1 > jN) or (j1 < 1) then
      begin
        ShowMessage(pc005_110_132);
        Exit;
      end;
    end
    else
  ...
  fgWind := self.fImage1[jN].gWd;
//  fgWind := self.fImage1[self.fImage1.findWindowList(self.numList)].gWd;
  uEd_Par.Init(fjob, self.Intervals, fgWind);
  uEd_Par.ShowModal;
end;

procedure TcShowImage3.StringSign;
begin
  uSupport.prqClearGrid(self.StringGrid1);
...
procedure TcShowImage3.StringParShow;
var
  j1, jC1: Integer;
  pg: TprqRptGraf1;
  DTime: TDateTime;
...
begin
  ...
  if self.DescriptMouse.pntBGraf then
  begin
    for jC1 := 1 to self.DescriptMouse.fgWind.Grafiks.Count do
    begin
      pg := self.DescriptMouse.fgWind.Grafiks[jC1].ukz as TprqRptGraf1;
      if pg.bActive then
      begin
        ...
        if j1 > 0 then
        begin
          DTime := abs(DTime - self.DescriptMouse.dTime) * 86400000; // = 24 * 3600 * 1000;
          if self.fjob.DEPGrhDescr.Pages.sizeTOUT > 0 then
          begin
            if DTime >= self.fjob.DEPGrhDescr.Pages.sizeTOUT then continue;
          end;

          self.StringGrid1.Cells[0,jC1] := Trim(pg.sName);
...
procedure TcShowImage3.RepaintImage2(Sender: TObject);
begin
  IntervalRepaint;
...
procedure TcShowImage3.BitBtn6Click(Sender: TObject);
var
  jC1, j1: Integer;
begin
  inherited;
...
//  self.ComboBox2.Items.Clear;
  ...
  begin
    ...
    // Формируем список треков на всякий случай
...
procedure TcShowImage3.CreateAllImage;
var
  myImage1: TImage;
  dHeight, dWidth: Double;
  jCrt, PixPerInch, jcScale, jNom: Integer;
//  bSearch: boolean;
//  sCl: double;
//  txtScale: double;
begin
  jCrt := self.fjob.gList.LogOtsBottom;
  try

    if self.fMode = 0 then
    begin
  // Экспорт графиков в Excel
      dWidth  := Screen.PixelsPerInch *  8.3;
      dHeight := Screen.PixelsPerInch * 11.7;
      self.fjob.gList.LogWidth  := Round( (dWidth  * pc005_505_LogInch) / Screen.PixelsPerInch );
      self.fjob.gList.LogHeight := Round( (dHeight * pc005_505_LogInch) / Screen.PixelsPerInch );
      self.fjob.gList.LogOtsBottom := 5;
  //    bSearch := true;
    end
    else
    begin
  // на экран
      ...
      self.fjob.gList.LogWidth  := Round( (dWidth  * pc005_505_LogInch) / Screen.PixelsPerInch );
      self.fjob.gList.LogHeight := Round( (dHeight * pc005_505_LogInch) / Screen.PixelsPerInch );
  //    bSearch := self.FdepImgParam.crntStepIndx < 0;
    end;

    self.fjob.pctWidth  := Round(dWidth);
    self.fjob.pctHeight := Round(dHeight);
    PixPerInch := Screen.PixelsPerInch;
      ...
      begin // Подгонка начального шага
  {
    ...
    if self.FdepImgParam.crntStepIndx = 4 then // шаг =  4 часа          4  2
        ...
        begin
          ...
          fJob.gList.getTimeRegionM(myImage1.Picture.Bitmap.Canvas, //myImage1.Canvas,
  self.FdepImgParam.crntStepIndx, PixPerInch, self.dtCrntBeg, self.dtCrntEnd,
  self.outCrntBeg, self.outCrntEnd,
  myImage1.Picture.Bitmap.Width, myImage1.Picture.Bitmap.Height);
          ...
          begin
            ...
            fJob.gList.imageListU2M(myImage1.Picture.Bitmap,
  self.fjob.OtherParams, self.FdepImgParam.crntStepIndx, PixPerInch, self.dtCrntBeg, self.dtCrntEnd,
  self.outCrntBeg, self.outCrntEnd, 0, 0);
            break;
          end;
        end;
      end
  ...
  self.outCrntBeg, self.outCrntEnd, 0, 0);
    ...
    self.fjob.gList.LogOtsBottom := jCrt;
  end;
end;

procedure TcShowImage3.ImageTransfer(num: Integer);
var
  jNom: Integer;
  Dest: TRect;
begin
...
procedure TcShowImage3.BitBtn2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
  self.bigStep := self.CheckBox1.Checked;
...
procedure TcShowImage3.BitBtn3KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
  self.bigStep := self.CheckBox1.Checked;
...
procedure TcShowImage3.unknowDialogMessage(var Msg: TMessage;
  kode: Integer);
begin
...
function TcShowImage3.isBegOfStage(eNum: Integer; dTime: Double): Boolean;
var
  dBeg, dEnd: Double;
begin
  if eNum <= 0 then
...
function TcShowImage3.isBegOfIntrval(dTime, jobBeg, jobEnd: Double): Boolean;
var
  dBeg, dEnd: Double;
begin
...
procedure TcShowImage3.BitBtn7Click(Sender: TObject);
var
  pDD: TcDBVgrafQuery;
  j1: Integer;
begin
...
procedure TcShowImage3.BitBtn9Click(Sender: TObject);
begin
  inherited;
  if self.Intervals.Count = 0 then Exit;
  IntervalRepaint;
...
procedure TcShowImage3.AddCurrentState;
var
  jC1, j1: Integer;
begin
...
function TcShowImage3._CreateImage(pctWidth, pctHeight: integer): TImage;
var
  r: TRect;
begin
...
procedure TcShowImage3.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
{
...
function TcShowImage3.getStepDate(jNum: integer): double;
var
  sc: TprqTimeScale1;
begin
...
procedure TcShowImage3.BitBtn1Click(Sender: TObject);
var
  delta: double;
begin
  inherited;
...
procedure TcShowImage3.BitBtn8Click(Sender: TObject);
var
  delta: double;
begin
  inherited;
...
procedure TcShowImage3.SetScrollBar;
begin
  self.bScroll := true;
  try
...
procedure TcShowImage3.setMaxScrollBar;
var
  delta: double;
  j1: integer;
begin
...
procedure TcShowImage3.ScrollBar1Change(Sender: TObject);
var
  dtEnd: double;
begin
  inherited;
...
procedure TcShowImage3.BitBtn10Click(Sender: TObject);
var
  j1, jM: integer;
  sErr: string;
begin
...
function TcShowImage3.getImagePageCount(var sErr: string): integer;
var
  myImage1: TImage;
  dHeight, dWidth: Double;
  FpctHeight, FpctWidth, jC1: Integer;
  FdtCrntBeg, FdtCrntEnd, FdtBeg, FdtEnd: double;
  PixPerInch: integer;
begin
  ...
  self.fJob.gList.LogHeight    := self.fJob.DEPGrhDescr.Pages.Height;
  self.fJob.gList.LogWidth     := self.fJob.DEPGrhDescr.Pages.Width;

  PixPerInch := fJob.gList.getPrinterResolution(Printer.Canvas, FpctHeight, self.fjob.gList.LogHeight);
    ...
    begin
      ...
      try
        ...
        try
          ...
          begin
            fJob.gList.imageListU2M(myImage1.Picture.Bitmap,
self.fjob.OtherParams, self.FdepImgParam.crntStepIndx, PixPerInch,
FdtCrntBeg, FdtCrntEnd, FdtBeg, FdtEnd, 0, 0);
            FdtCrntBeg := FdtEnd;
            Inc(result);
          end;
        except
...
procedure TcShowImage3.printImagePage(jMode, n1, n2, maxPage: integer);
var
  dHeight, dWidth: Double;
  jn1, jn2, jCrntPageNum, FpctHeight, FpctWidth: Integer;
  FdtCrntBeg, FdtCrntEnd, FdtBeg, FdtEnd: double;
  sErr: string;
  bFirst: boolean;
  PixPerInch: integer;
...
//                       ShowMessage(IntToStr(jCrntPageNum));
            ...
            case jMode of
              ...
              0: begin
                   ...
                   fJob.gList.printListU2M(Printer.Canvas,
self.fjob.OtherParams, self.FdepImgParam.crntStepIndx, PixPerInch,
FdtCrntBeg, FdtCrntEnd, FdtBeg, FdtEnd, jn1, jn2, FpctWidth, FpctHeight);
                   bFirst := false;
                 end;
...
procedure TcShowImage3.BitBtn11Click(Sender: TObject);
begin
  inherited;
  if self.PrintDialog1.Execute then
  begin
...
procedure TcShowImage3._ClearCanvas(canva: TCanvas; pctWidth, pctHeight: integer);
var
  r: TRect;
begin
  r.Left   := 0;
...
function TcShowImage3._findWindow(pnt1: TPoint): TprqRptGrafWindow;
var
  wnd: TRect;
  fgWind: TprqRptGrafWindow;
  jC1: integer;
begin
  result := nil;
  for jC1 := 1 to self.fImage1.Count do
  begin
    fgWind := self.fImage1[jC1].gWd;
    ...
    if not TprqAGallfunc.isInternalPoint(pnt1, wnd, False) then continue;
    result := fgWind;
    Exit;
  end;
end;
